...
export const getMovedMino = (m: Mino, x: number, y: number): Mino => {
  const newBlocks = m.blocks.map(
    (block) => [block[0] + x, block[1] + y] as [number, number]
  );
  return {
    ...m,
    blocks: newBlocks,
...
export const getRotatedMino = (m: Mino, n: number): Mino => {
  ...
  for (let i = 0; i < numRotations; i++) {
    newBlocks = newBlocks.map(
      (block) =>
        [block[1] - m.oy + m.ox, -(block[0] - m.ox) + m.oy] as [number, number]
    );
  }
...
export const getBlocksFromString = (s: string): Blocks => {
  ...
  const lines = s.split("\n").reverse();
  const maxWidth = lines.reduce((prev, curr) =>
  ...
  return lines.map((line) => {
    let result = line
      .split("")
      .map((c) =>
        blockTypesArray.includes(c.toUpperCase()) ? c.toUpperCase() : ""
      );
