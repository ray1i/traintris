...
const areInputsEqual = (a: Input, b: Input): boolean => {
  ...
  if (
    a.queue.length + (a.hold === null ? 0 : 1) !=
    b.queue.length + (b.hold === null ? 0 : 1)
  ) {
    return false;
  }
  ...
  if (a.queue.length + (a.hold === null ? 0 : 1) <= 1) {
    return (
      (a.hold === b.hold && a.queue[0] === b.queue[0]) ||
      (a.hold == b.queue[0] && a.queue[0] == b.hold)
    );
  }
